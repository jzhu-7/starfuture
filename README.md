...
- 自动抓取
  ...
  - 抓取房源面积（`core/scrapers/area_scraper.py`）：解析房源详情页并生成 `data/{project}/areas/areas.json`（项目分为 `house` 与 `warehouse`，默认 `house`）。
...
- 可视化与交互（Streamlit）
  ...
  - 侧边栏支持手动更新（点击“更新数据”会调用 `core.main` 执行抓取与处理流程）、项目类型切换（住宅/仓储，采用横向单选切换）与日期选择器。注：页面已移除“更新面积”按钮，如需更新面积请使用 CLI（见下文）。
...
时区与时间处理
----------------
- 部署时建议在运行环境中显式设置 `TZ=Asia/Shanghai`（或你想要的时区），并在 Unix 系统上调用 `time.tzset()`。仓库已添加 `core/utils/time_utils.py` 并在 `app.py` / `core/main.py` 中启用默认时区为 `Asia/Shanghai`，简化在不同宿主机上的时间一致性问题。
- 最佳实践：后端统一以 UTC 存储时间戳，前端在展示时或根据用户时区转换显示；所有代码应优先使用时区感知的 `datetime` 对象（项目已将常见地方替换为时区感知的当前时间）。
...
- 更新销售数据（可指定项目，默认 `house`）：

```bash
python core/main.py data [project]
# 例如： python core/main.py data house
```

- 更新面积数据（可指定项目）：

```bash
python core/main.py areas [project]
# 例如： python core/main.py areas warehouse
```
...
- 在启用 Streamlit 时可以通过环境变量改变默认项目：

```bash
# 设置默认项目为 warehouse 后启动
export PROJECT_TYPE=warehouse
...
- `data/{project}/total.json`：汇总后的总数据（每个项目独立）
- `data/{project}/areas/areas.json`：面积相关数据（每个项目独立）
- `data/{project}/sales/YYYY-MM-DD.json`：按日期保存的每日销售数据（每个项目独立）
